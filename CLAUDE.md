...
- **New tools:** `get_method_body`, `get_method_smali`, `get_inner_classes`, `resolve_class`; enhanced: `get_class_source` + `get_method_by_name` (`smali_fallback`), `get_smali_of_class` (`offset` paging with `total_bytes`/`next_offset`).

### Resource export (v0.4.0)

- **`export_apk_resources` / `export_arsc_resources`** write decoded resources to a directory on disk — the same operation as the jadx GUI's right-click *"Export"* on a resource node. Both are thin wrappers over jadx-core's own `jadx.core.xmlgen.ResourcesSaver` (the exact Runnable the GUI export runs), driven from `JadxSession.exportFileResources` / `exportArscResources`. `ResourcesSaver` walks each `ResourceFile`'s `ResContainer` and dispatches by `DataType`: TEXT (decoded AXML/values) → `SaveCode.save`, DECODED_DATA → `Files.write`, RES_LINK (drawables/raw/fonts) → verbatim `Files.copy` from the zip, RES_TABLE → recurse into sub-files; path-traversal is blocked by jadx itself.
- **The two are complementary**, matching the two `res` nodes in the GUI tree: `export_apk_resources` = the top-level **file-based** `res/` (every `ResourceFile` whose `deobfName` starts with `res/`; `prefix` arg overridable, e.g. `assets/`); `export_arsc_resources` = the `res` subtree under `resources.arsc` = the **table-decoded** `res/values*` + `public.xml` that exist only inside the arsc binary, not as standalone entries.
- Export runs on the single decompile worker under a wide wall-clock budget (`DEFAULT_EXPORT_TIMEOUT_MS = 300_000`, `timeout_ms` overridable). Response returns `out_dir` (default `<apk_dir>/<apk_name>.apk_res` / `.arsc_res`), `sources_scoped`, `file_count`, `total_bytes`, a `manifest_limit`-capped `files` sample, and any per-file `errors`. **These tools write to the local disk** (client and server share the filesystem). 28 tools total.
